...
const BlogDetails = ({ params }) => {
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://ziprus-blog.vercel.app/api/blog/${params.id}/comment`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${session?.user?.accessToken}`,
  ...
  const handleDeleteComment = async(commentId) => {
    try {
      const response = await fetch(
        `https://ziprus-blog.vercel.app/api/blog/${params.id}/comment/${commentId}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${session?.user?.accessToken}`,
    ...
    <section className="container max-w-3xl ">
      {blogDetails?.authorId?._id.toString() ===
        ...
        <div className="flex items-center justify-end gap-5">
          <Link
            ...
            className="flex items-center gap-1 text-base_color"
          >
            <BsFillPencilFill />
            Edit
          </Link>
      ...
      <div className="flex flex-col items-center justify-center">
        <Link href={`/user/${blogDetails?.authorId?._id.toString()}`}>
          <div className="flex flex-col justify-center items-center py-10">
            ...
            <div className="text-center">
              <p className="text-base_text">{blogDetails?.authorId?.name}</p>
        ...
        <div className="text-center space-y-3">
          <h2 className="text-base_two">{blogDetails?.title}</h2>

          <p className="text-gray-700">{blogDetails?.excerpt}...</p>

          <p className="flex items-center justify-center gap-3">
            <span className="text-base_two">{blogDetails?.category}</span>
                  ...
                  (paragraph, pIndex) => (
                    <div key={pIndex} className="">
                      {pIndex ===
                        ...
                        <blockquote className="border-l-4 border-base_two border-spacing-6 italic mb-5 text-gray-700">
                          <p className="ml-5 text-gray-700">{blogDetails?.quote}</p>
                        </blockquote>
                      )}
      ...
      <div>
        {!session?.user && (
          <h3 className="text-red-500">Kindly <Link href={"/login" } className="text-red-900">login</Link> to leave a comment.</h3>
        )}
        ...
        {session?.user && (
          <form onSubmit={handleCommentSubmit} className="space-y-2">
            ...
            <button type="submit" className="btn bg-base_two">
          ...
          <>
            {blogDetails.comments.map((comment) => (
              <div key={comment._id} className="flex gap-3 py-5 items-center">
                ...
                <div>
                  <p className="text-base_text">{comment?.user?.name}</p>
